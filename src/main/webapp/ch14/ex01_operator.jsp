...
<body style="margin: 40px" style="margin-bottom: 40px">
	<h1>EL 연산자</h1><hr>
	<table border="1">
		<tr><th>계산식</th><th>결과</th></tr>
		<tr><td>\${100}</td><td>${100}</td></tr>
		<tr><td>\${"안녕하세요"}</td><td>${"안녕하세요"}</td></tr>
		<tr><td>\${10 + 1 }</td><td>${10 + 1 }</td></tr>	<%-- "10"이 숫자으로 변환 --%>
		...
		<tr><td>\${10 >= && 8 <= 10 }</td><td>${10 >= 8 && 8 <= 10 }</td></tr>
		<tr><td>\${10 ge 8 && 8 le 10 }</td><td>${10 ge 8 && 8 le 10 }</td></tr>
